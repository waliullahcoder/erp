...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        if (auth()->attempt(array('user_name' => $input['user_name'], 'password' => $input['password']))) {
            return redirect()->route('client.dashboard.index')->with('success', 'Logged in Successfully!');
        } else {
    ...
    public function dashboard()
    {
        ...
        $sales = Sales::where('client_id', $client_id)->sum(DB::raw('total_amount - discount'));
        $sales_return = SalesReturnList::where('client_id', $client_id)->sum('amount');
        $collection = Collection::where('client_id', $client_id)->where('collection_type', '!=', 'adjust')->where('on_return', 0)->sum('amount');
        $info = [
            'sales' => $sales - $sales_return,
            'collection' => $collection,
            'balance' => $collection + $sales_return - $sales,
        ];
        return view('client.profile.dashbaord', compact('info'));
    }
    ...
    {
        $admin = Auth::user();
        return view('client.profile.index', compact('admin'));
    }
