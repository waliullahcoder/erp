    ...
    <div class="row g-3">
        ...
        <div class="col-lg-3 col-md-4 col-sm-6">
            ...
            <input type="text" class="form-control date_picker" id="date" name="date" required
                value="{{ date('d-m-Y', strtotime($data->date)) }}" placeholder="Prepare Date">
        </div>
        <div class="col-lg-3 col-md-4 col-sm-6">
                ...
                <table class="table table-bordered align-middle target-table mb-0 table-sm">
                    ...
                    <tbody id="selected">
                        @foreach ($data->list as $item)
                            <tr class="add_tr" id="remove_row_{{ $item->sales_list_id }}" data-type="remove"
                                data-id="{{ $item->sales_list_id }}">
                                <td class="p-0"><button type="button" class="btn move_btn custom-add-btn"
                                        id="btn_{{ $item->sales_list_id }}" data-id="{{ $item->sales_list_id }}"
                                        data-type="remove"><i class="fas fa-caret-left"></i></button></td>
                                <td class="px-3">
                                    {{ @$item->sales->invoice }}
                                    <input type="hidden" class="invoice_{{ $item->sales_list_id }}"
                                        value="{{ @$item->sales->invoice }}">
                                </td>
                                <td class="px-3">
                                    {{ @$item->sales_item->client->name }}
                                    <input type="hidden" class="client_name_{{ $item->sales_list_id }}"
                                        value="{{ @$item->sales_item->client->name }}">
                                </td>
                                <td class="px-3 text-nowrap">
                                    {{ @$item->sales_item->product->code }}
                                    <input type="hidden" class="product_code_{{ $item->sales_list_id }}"
                                        value="{{ @$item->sales_item->product->code }}">
                                </td>
                                <td class="px-3">
                                    {{ @$item->sales_item->product->name }}
                                    <input type="hidden" class="product_name_{{ $item->sales_list_id }}"
                                        value="{{ @$item->sales_item->product->name }}">
                                </td>
                                <td class="px-3 text-center">
                                    {{ @$item->sales_item->qty }}
                                    <input type="hidden" class="qty_{{ $item->sales_list_id }}"
                                        value="{{ @$item->sales_item->qty }}">
                                    <input type="hidden" name="sales_list_id[]" class="added_id"
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <th colspan="4"></th>
                            <th class="text-center"><span id="total2">{{ $data->list->sum('qty') }}</span></th>
                        </tr>
                    </tfoot>
                </table>
            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            function getSales() {
                ...
                let url = "{{ Route('admin.delivery.edit', $data->id) }}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
