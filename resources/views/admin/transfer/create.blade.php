    ...
    <div class="row g-3">
        {{-- <div class="col-md-4 col-sm-6">
            ...
            <select name="product_type" id="product_type" class="select form-select" data-placeholder="Select Product Type"
                required>
                <option value="Consumer" {{ old('product_type') && old('product_type') == 'Consumer' ? 'selected' : '' }}>
                    Consumer
                </option>
                <option value="Fashion" {{ old('product_type') && old('product_type') == 'Fashion' ? 'selected' : '' }}>
                    Fashion
                </option>
            </select>
        </div> --}}
        <div class="col-md-3 col-sm-6">
            ...
            <input type="text" class="form-control" id="transfer_no" name="transfer_no" placeholder="Transfer No"
                value="{{ $transfer_no }}" readonly required>
        </div>
        <div class="col-md-3 col-sm-6">
            ...
            <input type="text" class="form-control date_picker" id="date" name="date"
                value="{{ date('d-m-Y', strtotime(old('date'))) }}" placeholder="Transfer Date" required>
        </div>
        <div class="col-md-3 col-sm-6">
            ...
            <select name="host_id" id="host_id" class="select form-select" data-placeholder="Select Host Store" required>
                ...
                @foreach ($stores as $store)
                    <option value="{{ $store->id }}"
                        {{ old('host_id') && old('host_id') == $store->id ? 'selected' : '' }}>{{ $store->name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="col-md-3 col-sm-6" id="product_area">
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-striped target-table align-middle mb-0">
                    <thead class="bg-primary border-primary text-white align-middle text-nowrap">
                        <tr>
                            <th>Product Name</th>
                            <th width="220" id="changeable_text">Code</th>
                            <th width="220" class="text-center">Stock Qty</th>
                            <th width="220" class="text-center">Transfer</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('change', '#host_id', function(e) {
                ...
                let url = "{{ Route('admin.transfer.create') }}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: {
                        ...
                        success: (response) => {
                            if (response.status == 'success') {
                                ...
                                <tr id="product_${response.data.product.id}">
                                    ...
                                    <td>
                                        <input type="number" style="width: 120px;" class="form-control text-center mx-auto" name="stock_qty[${response.data.product.id}]" value="${stock}" step="any" readonly required>
                                    </td>
                                    <td>
                                        <input type="number" style="width: 120px;" class="form-control text-center mx-auto" name="transfer_qty[${response.data.product.id}]" value="${qty}" max="${stock}" step="any" required>
                                    </td>
                        ...
                        success: (response) => {
                            if (response.status == 'success') {
                                ...
                                <tr id="variant_${response.data.variant.id}">
                                    ...
                                    <td>
                                        <input type="number" style="width: 120px;" class="form-control text-center mx-auto" name="stock_qty[${response.data.variant.id}]" value="${stock}" step="1" readonly required>
                                    </td>
                                    <td>
                                        <input type="number" style="width: 120px;" class="form-control text-center mx-auto" name="transfer_qty[${response.data.variant.id}]" value="${qty}" max="${stock}" step="1" required>
                                    </td>
                                    <td class="text-center">
