                ...
                <div class="card-header pe-2 py-2">
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <div class="d-flex flex-wrap gap-2">
                            <a href="{{ Route(@$previous_route ?? 'admin.dashboard') }}" class="btn btn-primary btn-sm">Go
                                Back</a>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="row g-3 align-items-center">
                                ...
                                <div class="col-8">
                                    <table class="table invoice-table table-borderless mb-0 text-end">
                                        <tbody>
                                            <tr>
                                                <td class="p-0">{{ @$setting->address ?? '' }}
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="p-0"><a href="mailto:{{ @$setting->primary_email ?? '' }}"
                                                        target="_top">{{ @$setting->primary_email ?? '' }}</a>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td class="p-0"><a href="tel:{{ @$setting->primary_mobile ?? '' }}"
                                                        target="_top">{{ @$setting->primary_mobile ?? '' }}</a>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                                ...
                                <div class="invoice-address">
                                    ...
                                    <form action="{{ Route('admin.order-dashboard.update', $order->id) }}" method="POST"
                                        ...
                                        @elseif ($order->status != 'Delivered')
                                            <div class="alert-items d-flex gap-3 align-items-center mb-2">
                                                <label class="form-label flex-shrink-0 mb-0" style="width: 100px;"><b>Status
                                                        :
                                                    </b></label>
                                                <select name="status" class="form-select fs-14 px-2 py-1" id="status"
                                                    ...
                                                    @endif
                                                    @if ($order->status != 'Returned' && $order->status != 'On Route')
                                    ...
                                    <div id="courier_area">
                                        @if ($order->courier_assigned == 1)
                                            <span class="text-nowrap btn btn-xs text-white px-2 bg-success"
                                                style="min-width: 80px;">Courier Assigned</span>
                                        @elseif (in_array($order->status, ['Pending', 'On Route', 'Forward']))
                                            <a class="text-nowrap btn btn-xs text-white px-2 bg-primary assign_btn"
                                                style="min-width: 80px;"
                                                href="{{ Route('admin.online-order.edit', $order->id) }}">Assign to
                                                Courier</a>
                                        @endif
                                    </div>
                            ...
                            <table class="table table-bordered table-hover align-middle table-striped">
                                ...
                                <tfoot class="text-end">
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="1">{{ number_format($order->total - $order->discount) }}
                                            TK.</td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                if (response.status == 'success') {
                                    ...
                                    $('#courier_area').html(
                                        '<span class="text-nowrap btn btn-xs text-white px-2 bg-success" style="min-width: 80px;">Courier Assigned</span>'
                                    );
                                }
