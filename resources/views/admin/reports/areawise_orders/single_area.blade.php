            ...
            <form action="{{ @$filter_link }}" method="GET" class="filter_form">
                ...
                <div class="d-flex align-items-center justify-content-between gap-2">
                    ...
                    <div class="flex-shrink-0">
                        <a href="{{ Route('admin.areawise-orders.index') }}" class="btn btn-primary btn-sm">Go Back</a>
                    ...
                    <tbody>
                        @foreach ($data->orders->whereIn('status', ['Pending', 'Forward', 'Processing']) as $row)
                            <tr>
                                ...
                                <td class="px-3">
                                    <a class="btn btn-xs text-white px-2 {{ $row->status == 'Pending' ? 'bg-primary' : ($row->status == 'Forward' ? 'bg-info' : ($row->status == 'Processing' ? 'bg-warning' : '')) }}"
