                    ...
                    <tbody>
                        ...
                        @foreach ($data as $row)
                            <tr>
                                ...
                                <td class="px-3"><a
                                        href="{{ Route('admin.storewise-order.index') }}?store_id={{ $row->id }}">{{ @$row->name }}</a></td>
                                <td class="px-3 text-center">
                                    <a
                                        href="{{ Route('admin.storewise-order.index') }}?store_id={{ $row->id }}">{{ number_format(count($row->orders->whereIn('status', ['Pending', 'Forward', 'Processing', 'On Route', 'Delivered']))) }}</a>
                                </td>
                            </tr>
                            @php
                                $total_orders += count(
                                    $row->orders->whereIn('status', ['Pending', 'Forward', 'Processing', 'On Route', 'Delivered']),
                                );
                            @endphp
                        @endforeach
                    </tbody>
