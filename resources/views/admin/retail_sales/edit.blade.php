                    ...
                    <tbody id="tbody">
                        @foreach ($data->list as $item)
                            <tr>
                                ...
                                <td><input type="number" class="form-control rate" placeholder="Rate"
                                        id="rate_{{ $item->product_id }}" name="rate[{{ $item->product_id }}]"
                                        value="{{ $item->rate }}" readonly></td>
                                <td><input type="number" class="form-control discount" placeholder="Discount"
                                        id="discount_{{ $item->product_id }}"
                                        name="product_discount[{{ $item->product_id }}]"
                                        value="{{ $item->product_discount }}" readonly></td>
                                @php
                                    $stock = \App\HelperClass::stock($item->product_id, $data->store_id) + $item->qty;
                                @endphp
                                <td><input type="number" class="form-control qty" placeholder="Quantity"
                                        data-id="{{ $item->product_id }}" id="qty_{{ $item->product_id }}"
                                        name="qty[{{ $item->product_id }}]" value="{{ $item->qty }}" step="any"
                                        max="{{ $stock }}"></td>
                                <td><input type="text" class="form-control" value="{{ @$item->product->attribute->name }}" readonly></td>
                                <td><input type="number" class="form-control available_stock" placeholder="Available Stock"
                                        id="available_stock_{{ $item->product_id }}" value="{{ $stock - $item->qty }}"
                                        readonly></td>
                                <td><input type="number" class="form-control amount" placeholder="Amount"
                                        id="amount_{{ $item->product_id }}" name="amount[{{ $item->product_id }}]"
                                        value="{{ $item->amount }}" readonly></td>
                                <td class="text-center">
                                    <button type="button"
                                        class="btn btn-xs btn-outline-danger remove_item mnw-auto px-2">
                            ...
                            <td colspan="2">
                                <div class="input-group align-items-center flex-nowrap">
                                    <input type="number" id="total_amount" name="total_amount"
                                        class="form-control me-1" readonly placeholder="Total Cost"
                                        value="{{ $data->total_amount }}" style="min-width: 100px;">
                                    <b class="text-center" style="width: 25px;">TK.</b>
                                </div>
                                <div class="input-group align-items-center flex-nowrap">
                                    <input type="number" id="discount" name="discount" class="form-control me-1"
                                        placeholder="Discount" value="{{ $data->discount }}" style="min-width: 100px;">
                                    <b class="text-center" style="width: 25px;">TK.</b>
                                </div>
                                @if ($returnData)
                                ...
                                <div class="input-group align-items-center flex-nowrap">
                                    <input type="number" id="net_payable" name="net_payable" class="form-control me-1"
                                        ...
                                        value="{{ $data->total_amount - $data->discount - @$returnData->amount }}"
                                        style="min-width: 100px;">
                                    <b class="text-center" style="width: 25px;">TK.</b>
                                </div>
                                <div class="input-group align-items-center flex-nowrap">
                                    <input type="number" id="receive_amount" name="receive_amount"
                                        class="form-control me-1" placeholder="Cash Paid"
                                        value="{{ $data->receive_amount }}" style="min-width: 100px;">
                                    <b class="text-center" style="width: 25px;">TK.</b>
                                </div>
                                <div class="input-group align-items-center flex-nowrap">
                                    <input type="number" id="change_amount" name="change_amount"
                                        class="form-control me-1" readonly placeholder="Change Amount"
                                        value="{{ -$data->total_amount + $data->discount + $data->receive_amount + @$returnData->amount }}"
                                        style="min-width: 100px;">
                                    <b class="text-center" style="width: 25px;">TK.</b>
                                </div>
                            </td>
        ...
        $(document).ready(function() {
            ...
            function calculate() {
                ...
                if (discount_type == 'percentage') {
                    ...
                    $("#discount").val(Math.floor(fix_discount));
                    var net_payable = total_amount - Math.floor(fix_discount) - return_deduction;
                    $("#net_payable").val(net_payable);
                } else {
                    var discount = +$('#discount').val();
                    var net_payable = total_amount - discount - return_deduction;
                    $('#total_amount').val(total_amount);
                    $('#net_payable').val(net_payable);
                }
            }
