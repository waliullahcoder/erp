<aside class="bg-dark aside aside-fixed d-flex flex-column">
    <div class="brand">
        <a href="{{ Route('admin.dashboard') }}" class="brand-logo">
            <img class="lazyload"
                data-src="{{ $admin_setting && file_exists($admin_setting->logo) ? asset($admin_setting->logo) : asset('backend/images/logo/logo.png') }}"
                height="60" alt="Logo">
        </a>

        <button class="brand-toggle btn btn-sm px-0 d-lg-flex d-none @if (Session::has('sidebar-collapse')) active @endif">
            <span class="svg-icon svg-icon svg-icon-xl">
                <img class="lazyload"
                    data-src="{{ $admin_setting && file_exists($admin_setting->favicon) ? asset($admin_setting->favicon) : asset('backend/images/logo/favicon.png') }}"
                    height="40" alt="Logo">
